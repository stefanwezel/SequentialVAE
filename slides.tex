...
\end{itemize}
\begin{figure}
	\includegraphics[width=.6\linewidth]{figures/fm_variables.jpeg}
\end{figure}
\begin{tiny}
...
\mathcal{L}(\theta, \phi;x^{(n)}|\tilde{\mu_2}) &= \mathbb{E}_{q_{\phi}(z_1^{(n)},z_2^{(n)}|x^{(n)})}\left[ log\;p_{\theta}(x^{(n)}|z_1^{(n)},z_2^{(n)}) \right]\\
...
\begin{itemize}
	\item What is the sequence dependent prior $\mu_2$?
	\begin{itemize}
		\item imagine a word vector (->s-vector)
		\item $g(y) = \mu_2$ can be viewed as a differentiable lookup table (embedding in tf, pytorch)
	\end{itemize}
	\item regularize z2 by sequence dependent prior 
	\item and z1 by sequence independent prior  (i.e. standard Gaussian)
	\item $\mu_2$ can be found in closed form
...
\begin{frame}
\frametitle{FHVAE}
\framesubtitle{Self-supervised discriminative objective}
\begin{itemize}
	\item Train $z_2$ to predict sequence index (of sequence in lookup table)
\end{itemize}
\end{frame} 
...
\begin{figure}
	\includegraphics[width=.95\linewidth]{figures/timit_error.png}
